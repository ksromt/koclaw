...
from koclaw_agent.self_improving import LearningEntry, SelfImproving
...
class TestDetectCorrection:
    ...
    @pytest.mark.asyncio
    async def test_chinese_correction_start(self, si: SelfImproving):
        """Chinese correction keywords at start of message."""
        assert await si.detect_correction("不对，应该是这样的", "之前的回答")
        assert await si.detect_correction("错了，不是A而是B", "A是正确答案")
        assert await si.detect_correction("搞错了，我说的是X", "Y")
        assert await si.detect_correction("说错了吧", "之前说的")
        assert await si.detect_correction("不是这个意思，我要的是...", "你要说的是...")

    @pytest.mark.asyncio
    async def test_japanese_correction_start(self, si: SelfImproving):
        """Japanese correction keywords at start of message."""
        assert await si.detect_correction("違う、そっちじゃなくて", "前の回答")
        assert await si.detect_correction("間違いだよ、正しくは...", "前の回答")
        assert await si.detect_correction("そうじゃない、こっちだよ", "前の回答")
        assert await si.detect_correction("ちがう、それは間違い", "前の回答")

    @pytest.mark.asyncio
    async def test_english_correction_start(self, si: SelfImproving):
        """English correction keywords at start of message."""
        assert await si.detect_correction("that's wrong, it should be X", "prev response")
        assert await si.detect_correction("that's incorrect, the answer is Y", "prev")
        assert await si.detect_correction("not what I meant, I was asking about Z", "prev")
        assert await si.detect_correction("that's not right, try again", "prev")

    @pytest.mark.asyncio
    async def test_correction_after_punctuation(self, si: SelfImproving):
        """Correction keywords appearing after punctuation (not just at start)."""
        assert await si.detect_correction("嗯...不对，重新来", "回答")
        assert await si.detect_correction("hmm, that's wrong, try again", "response")
        assert await si.detect_correction("あの...違う、そうじゃなくて", "回答")

    @pytest.mark.asyncio
    async def test_false_positive_resistance_cn(self, si: SelfImproving):
        """Chinese: keywords embedded mid-sentence should NOT trigger."""
        # "Not right" embedded in a longer context about something else
        assert not await si.detect_correction("他说的不对你觉得呢", "回答")
        assert not await si.detect_correction("这个问题很不是我想的那样", "回答")

    @pytest.mark.asyncio
    async def test_false_positive_resistance_en(self, si: SelfImproving):
        """English: keywords buried in the middle should NOT trigger."""
        assert not await si.detect_correction(
            "I think the documentation says that's wrong approach for beginners", "response"
        )

    @pytest.mark.asyncio
    async def test_false_positive_resistance_jp(self, si: SelfImproving):
        """Japanese: embedded keywords should NOT trigger."""
        assert not await si.detect_correction("彼の意見は違うと思う", "回答")

    @pytest.mark.asyncio
    async def test_empty_messages(self, si: SelfImproving):
        """Empty messages should never detect correction."""
        assert not await si.detect_correction("", "response")
        assert not await si.detect_correction("hello", "")

    @pytest.mark.asyncio
    async def test_no_correction_normal_chat(self, si: SelfImproving):
        """Normal conversation should not trigger correction detection."""
        assert not await si.detect_correction("Thanks, that's helpful!", "response")
        assert not await si.detect_correction("ありがとう、助かった", "回答")
        assert not await si.detect_correction("好的，谢谢你", "回答")
...
class TestAutoPromote:
    ...
    @pytest.mark.asyncio
    async def test_promotion_writes_to_promotions_md(
        self, si: SelfImproving, learnings_dir: Path
    ):
        """Each promotion should be recorded in PROMOTIONS.md."""
        entry = LearningEntry(
            entry_type="ERR",
            priority="critical",
            area="test",
    ...
    @pytest.mark.asyncio
    async def test_normal_priority_does_not_promote_without_repeats(
        self, si: SelfImproving, knowledge_dir: Path
    ):
        """Normal priority with unique pattern_key should not promote."""
        entry = LearningEntry(
            entry_type="LRN",
            priority="normal",
            area="test",
            source="auto",
            summary="A normal learning",
            details="details",
            action="action",
            related_files=[],
            tags=[],
...
class TestRevokePromotion:
    ...
    async def test_revoke_without_confirmation_returns_prompt(
        ...
        # Extract the promo ID (first PROMO-... match)
        import re
    ...
    async def test_revoke_with_confirmation_removes_entry(
        ...
        promo_content = (learnings_dir / "PROMOTIONS.md").read_text(encoding="utf-8")
        assert "REVOKED" in promo_content

    @pytest.mark.asyncio
    async def test_revoke_nonexistent_promo(self, si: SelfImproving):
